...
export default function PinnedPage() {
  ...
  const MANY_POSTERS: PosterData[] = useMemo(() => { // useMemo() for cacheing
    const filler = makeRandomPosters(EXAMPLE_POSTERS.length, 200); // the integer value is how many random posters to make
  ...
  return (
    <View style={styles.container}>
      <PanBoard canvasWidth={canvas.w} canvasHeight={canvas.h} onViewportChange={setViewport}>
        <PackedScatterGrid
          items={items}
          spacing={8}
          compactness={0.25}
          padding={80}
          visibleRect={viewport ?? undefined} // <<— NEW
          overscan={200}                      // <<— NEW
          onContentSize={(w, h) => {
