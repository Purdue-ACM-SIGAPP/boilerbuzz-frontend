...
export function makeRandomPosters(baseCount: number, howMany: number): PosterData[] {
  ...
  for (let i = 0; i < howMany; i++) {
    ...
    out.push({
      id: `${baseCount + i + 1}`, // Making sure example poster id and filler poster id don't collide
      title: `Sample Poster ${baseCount + i + 1}`,
      image: require('https://picsum.photos/seed/${encodeURIComponent(id)}/400/600'),
      likes,
...
export function makePicsumPosters(count: number): PosterData[] {
  const out: PosterData[] = [];
  for (let i = 0; i < count; i++) {
    const id = `m${i + 1}`
    const likes = Math.random() * 600;
