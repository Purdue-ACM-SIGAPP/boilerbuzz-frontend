...
export default function PanBoard({
  ...
  // Sends updates of how much visible area moved by
  ...
  // Setting up panning/dragging gesture
  const pan = Gesture.Pan()
    .onBegin(() => { // Start of pan
      if (!measuredReady.value || !enabledSV.value) return; //checks screen was measure and pan is on

      // Remember where drag started
      startXSV.value = tx.value;
      startYSV.value = ty.value;

      // Remember last time panboard updated
    ...
    .onChange((e) => { 
      ...
      // every frame translate x and y based on where finger goes
      // Also uses clamp to make sure user doesn't drag outside of screen
      ...
      // Every 64 pixels, updates where visible area is for rendering posters
    ...
    // Stopping drag/pan
    .onEnd(() => { // update react one more time 
      if (!measuredReady.value || !enabledSV.value) return;
      notifyJS(tx.value, ty.value, viewW.value, viewH.value);
    })
  ...
  // This actually moves the canvas visually when you drag (ty and tx change)
