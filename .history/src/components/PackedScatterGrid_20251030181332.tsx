...
export default function PackedScatterGrid({
  ...
  // center the cluster inside that canvas
  const offsetX = Math.round((renderW - (layout.usedW ?? 0)) / 2 - (layout.minX ?? 0));
  const offsetY = Math.round((renderH - (layout.usedH ?? 0)) / 2 - (layout.minY ?? 0));
