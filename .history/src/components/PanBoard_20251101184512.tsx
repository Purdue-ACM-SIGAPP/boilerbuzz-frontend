...
/**
 * PanBoard (UI-thread panning with safety guards)
 *
 * - Pans a fixed-size canvas (canvasWidth x canvasHeight) inside a measured viewport.
 * - Clamps translations to prevent dragging beyond edges.
 * - Starts centered.
 * - Reports viewport (world coords) to JS, throttled, via scheduleOnRN.
 * - Guards against panning/notifications before measurements are ready.
 */
...
export default function PanBoard({
  ...
  // --- JS state: measured viewport size (area under headers/toolbars)
  ...
  // Latest callback ref so scheduled tasks don’t hold stale closures
  const onViewportChangeRef = useRef<typeof onViewportChange>();
  useEffect(() => {
    onViewportChangeRef.current = onViewportChange;
  }, [onViewportChange]);

  // --- Compute clamp bounds + centered start when viewport known
  const bounds = useMemo(() => {
    if (!viewport.w || !viewport.h) return null;

    // Canvas can be smaller than viewport; allow centering (min may be > 0 without clamp)
  ...
  // Ready flag: 0 = not ready, 1 = ready (prevents pan during load)
  const ready = useSharedValue(0);

  // Throttle JS notifications
  const lastSentX = useSharedValue(0);
  const lastSentY = useSharedValue(0);
  const THRESH = 64; // px before notifying JS again (tune 32–96)
  ...
  // Initialize shared values when bounds/viewport change
  useEffect(() => {
    if (!bounds) return;
    ...
    // Mark as ready only when we actually have non-zero viewport
    ready.value = (viewport.w > 0 && viewport.h > 0) ? 1 : 0;

    // Initial viewport notification (schedule on RN to avoid JS/Worklet crossing issues)
  ...
  // Pan gesture on UI thread with guards + throttled notifications
  const pan = Gesture.Pan()
    .onBegin(() => {
      ...
      if (onViewportChangeRef.current) {
        ...
        // Don’t emit if we don’t have a valid viewport yet
        if (w > 0 && h > 0) {
