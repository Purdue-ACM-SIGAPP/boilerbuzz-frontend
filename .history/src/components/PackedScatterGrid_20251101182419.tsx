...
const layout = useMemo(() => {
  ...
  const kyey = (gx: number, gy: number) => ((gx * 73856093) ^ (gy * 19349663)) >>> 0;
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    for (let ring = 0; ring < maxRings && !chosen; ring++) {
      ...
      const nAngles = Math.min(48, Math.max(12, Math.ceil(circumference / approxStep))); // cap to 48
      const deltaAngle = TAU / nAngles;
      const cosD = Math.cos(deltaAngle), sinD = Math.sin(deltaAngle);

      let cosT = Math.cos(startAngle);
      let sinT = Math.sin(startAngle);
