...
// Cheap integer hash key for (gx, gy)
...
export default function PackedScatterGrid({
  ...
  const layout = useMemo(() => {
    ...
    // --- Neighbor grid (numeric keys to avoid string churn) ---
    const minW = Math.max(1, Math.min(...items.map((d) => d.width)));
    const minH = Math.max(1, Math.min(...items.map((d) => d.height)));
    const cellSize = Math.max(32, Math.floor(Math.min(minW, minH) + spacing)); // slightly larger cell
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      const ringStep = baseRingStep * (1 + (0.6 - sizeScore)); // small → bigger steps
      ...
      for (let ring = 0; ring < maxRings && !chosen; ring++) {
        ...
        const nAngles = Math.min(48, Math.max(12, Math.ceil(circumference / approxStep))); // cap at 48
  ...
  // After the memo returns { placed, ... }
useEffect(() => {
  // 1) Every input item should be placed exactly once
  if (layout.placed.length !== items.length) {
    console.warn(
      `[PackedScatterGrid] Placed ${layout.placed.length} of ${items.length} items`
    );
  }

  // 2) Optional deeper check: ensure all ids are represented
  const placedIds = new Set(layout.placed.map(n => n.id));
  const missing = items.filter(it => !placedIds.has(it.id));
  if (missing.length) {
    console.warn(`[PackedScatterGrid] Missing IDs:`, missing.slice(0, 5), '…');
  }
}, [layout.placed, items]);
