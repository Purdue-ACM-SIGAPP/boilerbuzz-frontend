...
export default function PackedScatterGrid({
  ...
  const layout = useMemo(() => {
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      const ringStep = baseRingStep * (1 + (0.1 - sizeScore)); // small → bigger steps
...
// DEV logging (prints first 10, then every 25th, and the last)
if (__DEV__) {
  const count = i + 1;
  ...
  if (count <= 10 || count % 25 === 0 || count === items.length) {
